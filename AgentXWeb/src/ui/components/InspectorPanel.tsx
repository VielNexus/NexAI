...
import { AgentXDropdown } from "./AgentXDropdown";
...
export function InspectorPanel({ statusOk, statusName, chatProvider, chatModel, providerEndpointStatus, providerModelStatus, lastProviderError, retrieved, auditTail, verificationLevel, verification, webMeta, activeThreadId, unsafeStatus, onUnsafeStatus }: Props) {
  ...
  return (
    <Panel className="agentx-inspector flex min-h-0 flex-col gap-3 p-3">
      ...
      <ScrollArea className="min-h-0 flex-1 pr-1">
        <div className="space-y-3">
          <Panel className="agentx-inspector-card p-3">
            <div className={tokens.smallLabel}>Connection</div>
            <div className="agentx-kv mt-3 text-sm">
              <div>
                <div className="agentx-kv-label">API</div>
                <div className="agentx-kv-value font-mono text-[13px]">{config.apiBase}</div>
              </div>
              <div>
                <div className="agentx-kv-label">Status</div>
                <div className="agentx-kv-value">{statusOk ? statusName : "Offline"}</div>
              </div>
              <div>
                <div className="agentx-kv-label">Chat</div>
                <div className="agentx-kv-value">{`${chatProvider} / ${chatModel}`}</div>
              </div>
              {chatProvider === "ollama" ? (
                <>
                  <div>
                    <div className="agentx-kv-label">Provider</div>
                    <div className="agentx-kv-value">ollama</div>
                  </div>
                  <div>
                    <div className="agentx-kv-label">Endpoint</div>
                    <div className="agentx-kv-value">{providerEndpointStatus ?? "unknown"}</div>
                  </div>
                  <div>
                    <div className="agentx-kv-label">Model</div>
                    <div className="agentx-kv-value">{providerModelStatus ?? "unknown"}</div>
                  </div>
                  {lastProviderError ? (
              ...
              <div>
                <div className="agentx-kv-label">Verification</div>
                <span className="agentx-pill inline-flex rounded-full px-2.5 py-1 text-[11px]">
                  {verificationSummary}
                </span>
              </div>
              {providersUsed.length ? (
                <div>
                  <div className="agentx-kv-label">Search providers</div>
                  <div className="agentx-kv-value">{providersUsed.join(", ")}</div>
                </div>
              ) : null}
