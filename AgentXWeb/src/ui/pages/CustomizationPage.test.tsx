...
import { DEFAULT_AGENTX_SETTINGS } from "../../api/client";
...
describe("CustomizationPage", () => {
  ...
  it("rolls the visible draft back when customization save fails", async () => {
    ...
    render(
      <CustomizationPage
        statusOk
        settings={DEFAULT_AGENTX_SETTINGS}
    ...
    const assistantInput = screen.getByDisplayValue("AgentX");
    ...
    await waitFor(() => expect(screen.getByDisplayValue("AgentX")).toBeTruthy());
    expect(onSettingsChange).toHaveBeenLastCalledWith(expect.objectContaining({ assistantDisplayName: "AgentX" }));
  });
