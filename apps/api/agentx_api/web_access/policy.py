...
def validate_url(url: str, *, policy: WebPolicy) -> str:
    if not policy.enabled:
        raise WebAccessDenied("Web access is disabled (AGENTX_WEB_ENABLED=false).")
...
def validate_host(hostname: str, *, policy: WebPolicy) -> str:
    ...
    for suffix in policy.allowed_host_suffixes:
        ...
        if host == s or host.endswith("." + s):
            return host
    raise WebAccessDenied("Host is not in AGENTX_WEB_ALLOWED_HOSTS and AGENTX_WEB_ALLOW_ALL=false.")
