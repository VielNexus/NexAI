...
def _repo_root() -> Path:
    app_root = (os.environ.get("AGENTX_APP_ROOT") or os.environ.get("SOL_APP_ROOT") or os.environ.get("NEXAI_APP_ROOT") or "").strip()
    if app_root:
        return Path(app_root).expanduser().resolve()
    return Path(__file__).resolve().parents[3]


def _agentx_root() -> Path:
    return _repo_root() / "AgentX"
...
_HANDLE: AgentXHandle | None = None
_INIT_ERROR: str | None = None
_SESSION_WEB_ALLOWED: dict[tuple[str, str], set[str]] = {}


def _audit_agent_info(*, handle: AgentXHandle, summary: str, reason: str, meta: dict[str, Any] | None, success: bool, error: str | None) -> None:
    from agentx.core.audit import AuditEvent

    ok, err = handle.ctx.audit.ensure_writable()
    if not ok:
        raise AgentXUnavailable(f"AgentX audit log not writable: {err}")
    ...
    if not a_ok:
        raise AgentXUnavailable(f"AgentX audit append failed: {a_err}")
...
def get_handle() -> AgentXHandle:
    global _HANDLE, _INIT_ERROR
    with _LOCK:
        if _HANDLE is not None:
            return _HANDLE
        if _INIT_ERROR is not None:
            raise AgentXUnavailable(_INIT_ERROR)

        root = _agentx_root()
        if not root.exists():
            _INIT_ERROR = f"AgentX folder not found at {root}"
            raise AgentXUnavailable(_INIT_ERROR)

        # Ensure the AgentX package root is importable (contains `agentx/`).
        ...
        try:
            ...
            def confirm(_: str) -> bool:
                ...
                # and still enforces AgentX filesystem policy + journaling. Treat a user-initiated
            ...
            try:
                ...
                restore()
            _HANDLE = AgentXHandle(cfg=services.cfg, agent=services.agent, tools=services.tools, ctx=services.ctx, started_at=time.time())
            return _HANDLE
        except AgentXUnavailable:
            raise
        except Exception as e:
            tb = traceback.format_exc(limit=50)
            _INIT_ERROR = f"Failed to initialize AgentX: {e}\n{tb}"
            raise AgentXUnavailable(_INIT_ERROR)


def _config_path() -> Path:
    override = (os.environ.get("AGENTX_CONFIG_PATH") or os.environ.get("SOL_CONFIG_PATH") or os.environ.get("NEXAI_CONFIG_PATH") or "").strip()
    if override:
        return Path(override).expanduser().resolve()
    return _agentx_root() / "config" / "agentx.toml"


def _replace_or_insert_managed_block(text: str, *, block: str) -> str:
    begin = "# BEGIN AGENTX MANAGED WEB POLICY"
    end = "# END AGENTX MANAGED WEB POLICY"
    legacy_begin = "# BEGIN SOL MANAGED WEB POLICY"
    legacy_end = "# END SOL MANAGED WEB POLICY"
...
def session_allow_domain(*, thread_id: str, domain: str, reason: str, client_host: str | None, user: str | None) -> None:
    ...
    if key is None or not dom:
        raise AgentXUnavailable("thread_id and domain are required.")
    h = get_handle()
...
def session_clear(*, thread_id: str, reason: str, client_host: str | None, user: str | None) -> None:
    ...
    key = _session_key(user=user, thread_id=th)
    if key is None:
        raise AgentXUnavailable("thread_id is required.")
    h = get_handle()
...
def update_web_policy(
    *,
    ...
    path = _config_path()
    if not path.exists():
        raise AgentXUnavailable(f"AgentX config not found: {path}")

    # Load existing config through AgentX to get current policy values.
    restore = _with_cwd(_agentx_root())
    try:
    ...
    restore = _with_cwd(_agentx_root())
    try:
        from agentx.config import load_config as _load
...
def get_agent_for_thread(thread_id: str | None, *, user: str | None = None) -> tuple[AgentXHandle, Any]:
