...
from agentx.core.action_policy import ActionSelectionPolicy
...
from agentx.core.runtime_models import ArtifactContext, Plan, PlanStep
...
from agentx.tools.fs import FsDeleteTool, FsGrepTool, FsReadTool, FsWriteTool
...
def test_posix_absolute_write_prompt_is_tool_addressable(tmp_path: Path) -> None:
    agent, work_dir = _prepare_fs_write_agent(tmp_path)
    prompt = "create a file at /home/agentx/demo.txt with content hello"

    assert agent._extract_write_path(prompt) == "/home/agentx/demo.txt"
    assert agent._extract_path(prompt) == "/home/agentx/demo.txt"
    ...
    assert forced[0].arguments["path"].endswith(str(Path("home") / "agentx" / "demo.txt"))
