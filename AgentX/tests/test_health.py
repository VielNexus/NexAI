...
from agentx.install.lifecycle import collect_health_report
...
def test_health_api_running_reports_healthy(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "agentx.install.lifecycle.status_installation",
        lambda cfg: {"service_mode": "systemd-user", "services": {"api": {"state": "active"}, "web": {"state": "active"}}},
    )
    monkeypatch.setattr("agentx.install.lifecycle._http_health", lambda url, timeout_s=1.0: url.endswith("/v1/status"))
    monkeypatch.setattr(
        "agentx.install.lifecycle.probe_ollama_provider",
    ...
    monkeypatch.setattr("agentx.install.lifecycle._port_open", lambda host, port, timeout_s=0.2: True)
...
def test_health_api_down_reports_unhealthy(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._http_health", lambda url, timeout_s=1.0: False if url.endswith("/v1/status") else True)
    monkeypatch.setattr(
        "agentx.install.lifecycle.probe_ollama_provider",
        lambda base_url, model=None, timeout_s=2.0: type("Probe", (), {"status": "reachable", "base_url": base_url, "model": model, "model_available": True, "category": "connected", "detail": None})(),
    )
...
def test_health_web_reachable_reports_reachable(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._http_health", lambda url, timeout_s=1.0: True if url.endswith("/v1/status") else False)
    monkeypatch.setattr(
        "agentx.install.lifecycle.probe_ollama_provider",
        lambda base_url, model=None, timeout_s=2.0: type("Probe", (), {"status": "reachable", "base_url": base_url, "model": model, "model_available": True, "category": "connected", "detail": None})(),
    )
    monkeypatch.setattr("agentx.install.lifecycle._port_open", lambda host, port, timeout_s=0.2: True if port == 5173 else False)
    result = collect_health_report(config)
...
def test_health_stopped_service_summary(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._http_health", lambda url, timeout_s=1.0: False)
    monkeypatch.setattr("agentx.install.lifecycle._port_open", lambda host, port, timeout_s=0.2: False)
    result = collect_health_report(config)
...
def test_health_partial_runtime_with_healthy_api_is_degraded(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._http_health", fake_http_health)
    monkeypatch.setattr("agentx.install.lifecycle._port_open", lambda host, port, timeout_s=0.2: False)
    result = collect_health_report(config)
    assert result["services"]["summary"] == "partially running (systemd-user)"
    assert result["overall"] == "DEGRADED"
...
def test_health_cli_renders_human_readable_output(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        "agentx.cli.collect_health_report",
        lambda install: {
    ...
    assert code == 0
    assert "AgentX Health Check" in captured.out
