...
from agentx.install.lifecycle import apply_safe_doctor_fixes, compute_install_paths
...
def test_doctor_without_fix_remains_read_only(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.cli._print_doctor_report", lambda result: None)
    monkeypatch.setattr(
        "agentx.cli.run_doctor",
        lambda install: {
            "ok": True,
            "paths": {
    ...
    monkeypatch.setattr(
        "agentx.cli.apply_safe_doctor_fixes",
...
def test_doctor_fix_recreates_missing_runtime_directories(tmp_path: Path, monkeypatch) -> None:
    ...
    paths.web_config_path.write_text("window.AGENTX_CONFIG = {};\n", encoding="utf-8")
    launcher_path = tmp_path / "bin" / "agentx"
    launcher_path.parent.mkdir(parents=True, exist_ok=True)
    launcher_path.write_text("", encoding="utf-8")
    reports = iter([_doctor_report(paths, ok=False), _doctor_report(paths, ok=True)])
    monkeypatch.setattr("agentx.install.lifecycle.run_doctor", lambda cfg: next(reports))
    monkeypatch.setattr("agentx.install.lifecycle.collect_health_report", lambda cfg: _ok_health())
    monkeypatch.setattr(
        "agentx.install.lifecycle._managed_runtime_report",
        lambda cfg, p: {"required": False, "venv_exists": True, "python_exists": True, "dependencies": []},
    )
    monkeypatch.setattr("agentx.install.lifecycle.canonical_user_launcher_path", lambda: launcher_path)
...
def test_doctor_fix_regenerates_missing_runtime_and_web_config(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.ensure_installation_ready", _fake_ensure)
    result = apply_safe_doctor_fixes(config)
...
def test_doctor_fix_reprovisions_missing_managed_runtime(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._managed_runtime_report", _runtime_report)
    monkeypatch.setattr("agentx.install.lifecycle.ensure_installation_ready", _fake_ensure)
    result = apply_safe_doctor_fixes(config)
...
def test_doctor_fix_reinstalls_missing_dependencies(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._managed_runtime_report", _runtime_report)
    monkeypatch.setattr("agentx.install.lifecycle.ensure_installation_ready", lambda cfg: state.update(ready=True, ensure=1) or paths)
    result = apply_safe_doctor_fixes(config)
    assert state["ensure"] == 1
...
def test_doctor_fix_reinstalls_systemd_units_and_reloads(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.detect_platform", lambda: type("Platform", (), {"systemd_user_available": True})())
    monkeypatch.setattr("agentx.install.lifecycle._service_dir", lambda: service_dir)
    monkeypatch.setattr("agentx.install.lifecycle._systemctl_user_query", lambda *args, **kwargs: type("Proc", (), {"stdout": "enabled\n", "returncode": 0})())
    called = {"install": 0}
    ...
    def _fake_install(cfg, p):
        ...
        return [service_dir / "agentx-api.service", service_dir / "agentx-web.service"]

    monkeypatch.setattr("agentx.install.lifecycle.install_systemd_units", _fake_install)
    result = apply_safe_doctor_fixes(config)
...
def test_doctor_fix_enables_disabled_systemd_units(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.detect_platform", lambda: type("Platform", (), {"systemd_user_available": True})())
    monkeypatch.setattr("agentx.install.lifecycle._service_dir", lambda: service_dir)
    monkeypatch.setattr("agentx.install.lifecycle._systemctl_user_query", lambda *args, **kwargs: type("Proc", (), {"stdout": "disabled\n", "returncode": 1})())
    called = {"enable": 0}
    monkeypatch.setattr("agentx.install.lifecycle.install_systemd_units", lambda cfg, p: [])
    monkeypatch.setattr("agentx.install.lifecycle.enable_systemd_units", lambda cfg: called.update(enable=1) or {"agentx-api.service": "enabled", "agentx-web.service": "enabled"})
    result = apply_safe_doctor_fixes(config)
...
def test_doctor_fix_rewrites_missing_launcher(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.canonical_user_launcher_path", lambda: launcher_path)
    monkeypatch.setattr(
        "agentx.install.lifecycle.write_bootstrap_launcher",
    ...
    assert "Rewrote AgentX launcher." in result["fixes_applied"]
...
def test_doctor_fix_reports_remaining_ollama_problem_honestly(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.run_doctor", lambda cfg: next(reports))
    monkeypatch.setattr("agentx.install.lifecycle.collect_health_report", lambda cfg: next(health_reports))
    monkeypatch.setattr(
        "agentx.install.lifecycle._managed_runtime_report",
        lambda cfg, p: {"required": False, "venv_exists": True, "python_exists": True, "dependencies": []},
    )
