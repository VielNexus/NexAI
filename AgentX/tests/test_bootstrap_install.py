...
def test_default_bootstrap_paths_follow_xdg(tmp_path: Path) -> None:
    ...
    assert default_bootstrap_root(env=env, home=tmp_path) == (tmp_path / "xdg-data" / "agentx" / "bootstrap").resolve()
    assert default_runtime_root(env=env, home=tmp_path) == (tmp_path / "xdg-data" / "agentx").resolve()
    assert default_user_bin_dir(env=env, home=tmp_path) == (tmp_path / "xdg-bin").resolve()
    assert default_user_launcher_path(env=env, home=tmp_path) == (tmp_path / "xdg-bin" / "agentx").resolve()
...
def test_validate_app_bundle_reports_missing_components(tmp_path: Path) -> None:
    ...
    assert "AgentX/pyproject.toml" in missing
    assert "apps/api" in missing
    assert "AgentXWeb" in missing
...
def test_bootstrap_launcher_content_exports_bundle_root(tmp_path: Path) -> None:
    ...
    assert 'Re-run install-agentx.sh' in launcher
    assert 'AGENTX_BOOTSTRAP_APP_ROOT="' in launcher
    assert 'exec "$BOOTSTRAP_PY" -m agentx "$@"' in launcher
...
def test_launcher_targets_bootstrap_python_and_handles_spaces(tmp_path: Path) -> None:
    ...
    assert f'export AGENTX_BOOTSTRAP_APP_ROOT="{app_root.resolve(strict=False)}"' in launcher_text
    assert f'BOOTSTRAP_PY="{bootstrap_python.resolve(strict=False)}"' in launcher_text
    assert 'exec "$BOOTSTRAP_PY" -m agentx "$@"' in launcher_text


def test_write_bootstrap_launcher_writes_bootstrap_python_not_system_python(tmp_path: Path) -> None:
    launcher_path = tmp_path / ".local" / "bin" / "agentx"
    bootstrap_python = _make_bootstrap_python(tmp_path / "bootstrap" / "venv" / "bin" / "python")
    ...
    assert 'exec "$BOOTSTRAP_PY" -m agentx "$@"' in text


def test_write_bootstrap_launcher_supports_agentx_and_sol_aliases(tmp_path: Path) -> None:
    bootstrap_python = _make_bootstrap_python(tmp_path / "bootstrap" / "venv" / "bin" / "python")
    app_root = tmp_path / "bundle"
    agentx = write_bootstrap_launcher(
        launcher_path=tmp_path / ".local" / "bin" / "agentx",
        bootstrap_python=bootstrap_python,
        app_root=app_root,
    )
    ...
    assert agentx.name == "agentx"
    assert nexai.name == "nexai"
    assert sol.name == "sol"
    assert launcher_targets_bootstrap_python(launcher_text=agentx.read_text(encoding="utf-8"), bootstrap_python=bootstrap_python)
...
def test_launcher_validation_rejects_system_python_launcher(tmp_path: Path) -> None:
    launcher_text = (
        ...
        'export AGENTX_BOOTSTRAP_APP_ROOT="/bundle"\n'
...
def test_launcher_validation_rejects_exec_python3_pattern(tmp_path: Path) -> None:
    ...
    launcher_text = (
        ...
        f'export AGENTX_BOOTSTRAP_APP_ROOT="{app_root.resolve(strict=False)}"\n'
...
def test_shadowed_agentx_warning_reports_mismatch(tmp_path: Path) -> None:
    warning = shadowed_agentx_warning(
        launcher_path=tmp_path / ".local" / "bin" / "agentx",
        which_agentx=str(tmp_path / "other" / "agentx"),
    )
...
def test_bootstrap_setup_command_uses_bootstrap_python_not_path_sol(tmp_path: Path) -> None:
    ...
    assert cmd[:4] == [str((tmp_path / "bootstrap" / "venv" / "bin" / "python").resolve()), "-m", "agentx", "setup"]
...
def test_describe_command_renders_exact_setup_invocation(tmp_path: Path) -> None:
    ...
    assert "-m agentx setup" in rendered
...
def test_install_script_uses_bootstrap_python_for_post_install_helpers() -> None:
    script = (Path(__file__).resolve().parents[2] / "install-agentx.sh").read_text(encoding="utf-8")
    assert '"$BOOTSTRAP_PYTHON" - <<PY' in script
    assert '"$BOOTSTRAP_PYTHON" -m pip install --upgrade pip setuptools wheel' in script
    assert '"$BOOTSTRAP_PYTHON" -m pip install --upgrade "${APP_ROOT}/AgentX[cli]"' in script
    assert "python3 -m agentx" not in script
...
def test_setup_uses_bootstrap_app_root_env(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setenv("AGENTX_BOOTSTRAP_APP_ROOT", str(app_root))
    ...
    monkeypatch.setattr("agentx.cli.ensure_installation_ready", _fake_ensure)
    monkeypatch.setattr(
        "agentx.cli.inspect_runtime",
        lambda config: {"ready": True, "install_log_path": str(runtime_root / "logs" / "install.log")},
    )
    monkeypatch.setattr("agentx.cli.write_installation", lambda config, path: path)
    monkeypatch.setattr("agentx.cli.write_cli_launcher", lambda config, paths, install_cfg_path: runtime_root / "bin" / "agentx")
    monkeypatch.setattr("agentx.cli.save_local_profile", lambda runtime_root_arg, local_profile: runtime_root_arg / "config" / "profile.json")
    expected_launcher = Path("/home/test/.local/bin/agentx")
    expected_compat = Path("/home/test/.local/bin/sol")
    monkeypatch.setattr("agentx.cli.canonical_user_launcher_path", lambda: expected_launcher)
    monkeypatch.setattr("agentx.cli.compatibility_user_launcher_path", lambda: expected_compat)
    monkeypatch.setattr("agentx.cli.canonical_bootstrap_fallback", lambda: "/home/test/.local/share/agentx/bootstrap/venv/bin/python -m agentx")
    monkeypatch.setattr("agentx.cli.summary_panel", lambda title, items, style="#5B8CFF": ui_calls.__setitem__("summary", (title, list(items), style)))
    monkeypatch.setattr("agentx.cli.next_steps_panel", lambda title, steps, notes=None: ui_calls.__setitem__("next_steps", (title, list(steps), list(notes or []))))
    ...
    assert all(str(runtime_root / "bin" / "agentx") not in line for line in ui_calls["next_steps"][2])
